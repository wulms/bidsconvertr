...
#### Sessions
`r  json_merged_df %>% count(session)  %>% kable()` 
#### Sequences
`r  json_merged_df %>% count(relevant)  %>% kable()`
#### Relevant
`r  json_merged_df %>% filter(relevant == 1) %>% count(sequence)  %>% kable()`  
#### Irrelevant
`r  json_merged_df %>% filter(relevant == 0) %>% count(sequence)  %>% kable()`
#### Scanner information
`r  json_merged_df %>% count(ManufacturersModelName)  %>% kable()`  
`r  json_merged_df %>% count(Modality)   %>% kable()`  
`r  json_merged_df %>% count(MagneticFieldStrength)   %>% kable()` 
`r  json_merged_df %>% count(DeviceSerialNumber)   %>% kable()`  
`r  json_merged_df %>% count(SoftwareVersions)   %>% kable()`  
`r  json_merged_df %>% count(StationName) %>% kable()` 
`r  json_merged_df %>% count(InstitutionalDepartmentName)  %>% kable()`  
`r  json_merged_df %>% count(InstitutionName)  %>% kable()`  
...
json_merged_df %>%
  ...
  count() %>%
  select(BIDS_type, relevant, n, everything()) %>%
  datatable_setting()
