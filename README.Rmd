...
If you work on Windows just download [Rtools](https://cran.r-project.org/bin/windows/Rtools/rtools40.html) and install it. It is required to build packages.
...
Choose a folder, where you want to store your "user settings" file. The function `create_user_settings("path")` creates a template file at the desired folder. Take car, that the link contains a "/" at the end.
...
For more information on regular expressions (regex) please see the [stringR cheat sheet](https://github.com/rstudio/cheatsheets/blob/main/strings.pdf) or[RegexOne](https://regexone.com/). Each regex set here should match to your data. If you encounter any problems just contact me via mail or via the issues in this repository.
